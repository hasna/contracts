...
/**
 * Directories whose contents are BUILD OUTPUT — generated by a bundler, not
 * written by anyone.
 *
 * WHY THIS EXISTS. This scanner's own denylist is a pair of string literals:
 * `FORBIDDEN_SHARED_CLOUD_RUNTIMES = ["@hasna/cloud", "open-cloud"]`. Any repo
 * that imports `@hasna/contracts` without externalising it gets that array
 * INLINED into its bundle, and the scanner then read its own denylist back out
 * of the consumer's artifact and scored it as the consumer's breach. Every such
 * repo failed the gate permanently, and could not fix it by removing anything —
 * the only "fix" was to change bundling to please the scanner.
 *
 * The tell was `open-cloud` being reported against repos that never used it.
 *
 * `lib` is deliberately NOT here even though `SOURCE_DIRS` lists it: plenty of
 * repos author directly in `lib/`, and mistaking authored code for output would
 * cost a false negative. Erring toward noise is the correct direction.
 */
 ...
 *
 * Being in `bin/` is not on its own enough to call a file generated. `bin/` and
 * `build/` routinely hold hand-written shell scripts, and `shouldReadPath` reads
 * `.sh`, `.yml`, `.toml` and `.env` too. A `bin/deploy.sh` that exports
 * `HASNA_CLOUD_*` is somebody's decision, not a byte a bundler copied, and
 * exempting it would trade this false positive for a false negative on the one
 * pattern kind that has no import to look for.
 *
 * `.json` is excluded for the same reason, and because an inlined JS denylist
 * cannot land in one.
 */
...
function isGeneratedOutputPath(path: string): boolean {
  ...
  return segments.slice(0, -1).some((segment) => GENERATED_OUTPUT_DIRS.has(segment));
}
...
/**
 * The two AUTHORED files that declare the denylist, and so have to spell out
 * every name they forbid.
 *
 * This used to enumerate nineteen paths, most of them `dist/` bundles, and that
 * enumeration was the bug: every build-output file that could ever carry the
 * declaration had to be listed by hand, so the list was permanently one
 * bundler-entrypoint behind, and it could only ever describe THIS package's
 * output — never a consumer's, which is where the declaration actually lands.
 * Build output is now handled by `isGeneratedOutputPath` for every package
 * alike, so only the hand-written sources need naming here.
 */
const CONTRACTS_DECLARATION_PATHS = new Set(["src/no-cloud.ts", "src/schemas.ts"]);
...
function textFindings(file: ScanFile, severity: NoCloudFindingSeverity, packageName?: string): NoCloudFinding[] {
  ...
  /**
   * Is a BARE MENTION — the name, with no import resolving to it — evidence in
   * this file?
   *
   * Only where someone wrote it as this repo's own code. Two places fail that:
   *
   *   - build output, which contains whatever the bundler copied in from its
   *     dependencies, this scanner's denylist included;
   *   - the denylist's own authored declaration, which cannot name what it
   *     forbids without naming it.
   *
   * What this does NOT relax is the import itself. The checks below run in full
   * either way, so a specifier that still resolves to the retired runtime is a
   * finding in `dist/` exactly as in `src/` — verified: an externalised
   * `import { connect } from "@hasna/cloud"` survives `bun build` verbatim. So
   * does a dependency edge, which `package.json` and `bun.lock` answer on their
   * own and neither of which this touches.
   *
   * The previous spelling returned early for a declaration file, which meant a
   * REAL import inside one was exempt too. Narrowing the exemption to bare
   * mentions closes that.
   */
  const bareMentionIsEvidence = !isGeneratedOutputPath(file.path) && !isNoCloudDeclarationFile(file, packageName);
  ...
  for (const { pattern, kind, message } of RUNTIME_PATTERNS) {
    ...
    } else if (bareMentionIsEvidence && !(guardTest && kind === "module") && masked.includes(pattern)) {
