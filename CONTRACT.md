...
- A PostgreSQL capability declaration **MUST** include `storage.envPrefix`, so
  the serve/migrate boundary can derive `HASNA_<NAME>_DATABASE_URL`. It is not
  required while PostgreSQL is explicitly waived.
- `storage.pgTestGate` records the disposable live-Postgres test env var and
  command. Conformance records the command as data and never executes it. It is
  required whenever `postgres` is declared, and is not required while
  PostgreSQL is explicitly waived.

### Explicit storage-engine waivers

PostgreSQL remains the target for every store-owning core, but a CLI-only
`cli-with-store` repo MAY ship SQLite-only for an honest intermediate state by
declaring an explicit, auditable waiver instead of fabricating PostgreSQL
support:
...
- If `storage.mode` is `local`, **MUST** set `storage.sqlitePath`
  (`~/.hasna/<name>/<name>.db`).
- **MUST** declare `storage.pgTestGate` whenever `postgres` is declared; a
  waived PostgreSQL engine has no live-PG gate to declare.
...
8. `storage_capabilities` — store-owning cores declare SQLite + PostgreSQL (or
   explicitly waive PostgreSQL), `storage.envPrefix`, and a live-PG test gate
   for every declared PostgreSQL engine; SaaS declares its public PostgreSQL
   env prefix. An ineligible, non-waivable, or expired waiver fails the check.
...
4. declares SQLite + PostgreSQL capabilities, `storage.envPrefix`, and a
   ...
   `waivedStorageEngines` exception for a SQLite-only `cli-with-store`;
